...
from diffusion_policy.dataset.utils import rotation_utils as rtu
...
def rotation_transform(rot, from_rep, to_rep, from_convention=None, to_convention=None):
    ...
    """
    assert (
        from_rep in VALID_ROTATION_REPRESENTATIONS
    ), "Invalid rotation representation: {}".format(from_rep)
    assert (
        to_rep in VALID_ROTATION_REPRESENTATIONS
    ), "Invalid rotation representation: {}".format(to_rep)
...
def apply_mat_to_pose(pose, mat, rotation_rep, rotation_rep_convention=None):
    ...
    """
    assert (
        rotation_rep in VALID_ROTATION_REPRESENTATIONS
    ), "Invalid rotation representation: {}".format(rotation_rep)
    mat = np.array(mat)
    pose = np.array(pose)
...
def trans_rot_mat(offsets, angles):
    ...
    res = np.identity(4, dtype=np.float32)
    res[:3, :3] = rot_mat(angles)
    offsets = (res[:3, :3] @ np.expand_dims(np.array(offsets), axis=-1)).squeeze()
    res[:3, 3] = offsets
    return res


def trans_points(points, a2b_matrix):
    assert len(points.shape) == 2
    n = points.shape[0]
    points_homogeneous_in_a = np.hstack([points, np.ones((n, 1))])
    points_homogeneous_in_b = (a2b_matrix @ points_homogeneous_in_a.T).T  # nx4
    points_in_b = points_homogeneous_in_b[:, :3]
    return points_in_b
